...
# -------------------------------
def relu_poly(x):
    return x * 0.25 + 0.1  # effectively linear, keeps scale growth minimal
...
context = ts.context(
    ...
    coeff_mod_bit_sizes=[60, 40, 40, 40, 40, 60]  # extra level
)
context.global_scale = 2**25  # smaller initial scale
context.generate_galois_keys()

# -------------------------------
# 5. Extract FC weights and biases
# -------------------------------
w1 = (model.fc1.weight.data.cpu().numpy().astype(np.float64) / 20).tolist()
b1 = (model.fc1.bias.data.cpu().numpy().astype(np.float64) / 20).tolist()
w2 = (model.fc2.weight.data.cpu().numpy().astype(np.float64) / 20).tolist()
b2 = (model.fc2.bias.data.cpu().numpy().astype(np.float64) / 20).tolist()
# -------------------------------
...
print("TenSEAL: starting encrypted inference with polynomial ReLU...")
for idx, (images, labels) in enumerate(testloader):
    ...
    enc_fc1 = enc_x.mm(np.array(w1).T)  # shape (128,)
    enc_fc1 += b1
    ...
    enc_out = enc_fc1.mm(np.array(w2).T)  # shape (10,)
    enc_out += b2
    t_fc2_end = time.time()
